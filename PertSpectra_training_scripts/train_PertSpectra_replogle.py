...
def train(args):
    ...
    # replogle specific - filter to strong perturbations as done by Lopez et al, 2022
    filtered_replogle = read_aws_h5ad(
        "s3://pert-spectra/svae_replogle_k562_gw_dataset_with_nt_guides.h5ad"
    )
    filtered_perts = set(filtered_replogle.obs[args.perturbation_key].unique()).union(
        set(["SKP2", "CUL1", "UBE2N"])
    )
    adata = adata[adata.obs[args.perturbation_key].isin(filtered_perts)]
    ...
    if args.perturbation_key in adata.obs:
        D, pert_labels = vectorize_perts(adata, args.perturbation_key, args.control_key)
        pert_idx = np.array(
            [
        ...
        ctrl_vector = np.array(
            [
                1.0 if i == args.control_key else 0.0
                for i in adata.obs[args.perturbation_key]
            ]
        )
    ...
    train_idx, val_idx, _ = split_data_by_cell(X, adata.obs[args.perturbation_key])
    X_train = X[train_idx]
    D_train = D[train_idx]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--global_latent_dim", type=float, default=300)
    parser.add_argument("--seed", type=int, default=0)

    # cell type
    parser.add_argument("--use_cell_types", type=bool, default=False)
    # perturbation key if using perturbations
    parser.add_argument("--perturbation_key", type=str, default="gene")
    # control key
    ...
    # name of wandb run
    parser.add_argument("--experiment_name", type=str, default="pertspectra_replogle")

    parser.add_argument(
        "--anndata_s3_url",
        type=str,
        default="s3://pert-spectra/replogle_adata_spectra.h5ad",
    )
    parser.add_argument(
        "--model_save_s3_url",
        type=str,
